...
final class RuntimeRenderer
{
    public function __construct(
        private readonly TemplateRuntime $templates = new TemplateRuntime,
    ...
    public function render(array $page, array $blockRoots = [], array $context = [], ?array $registry = null, ?DatasourceResolver $datasource = null): RenderResult
    {
        $diagnostics = [];
        if (($page['version'] ?? $page['schemaVersion'] ?? 1) !== 1) {
            $diagnostics[] = (new Diagnostic('unsupported_page_version', 'error', '$.version'))->toArray();
            return new RenderResult('', ['css' => [], 'js' => []], $diagnostics);
        }
        try { $registry ??= $this->loader->load($blockRoots); }
        catch (Throwable $e) { return new RenderResult('', ['css' => [], 'js' => []], [(new Diagnostic('block_registry_error', 'error', '$.blocks', $e->getMessage()))->toArray()]); }
        $assets = ['css' => [], 'js' => []]; $seen = ['css' => [], 'js' => []];
        $body = '';
        foreach (($page['blocks'] ?? []) as $index => $block) if (is_array($block)) $body .= $this->renderBlock($block, $registry, $context, $datasource, $assets, $seen, $diagnostics, null, '$.blocks['.$index.']');
        return new RenderResult($this->wrapPage($page, $body), $assets, $diagnostics);
    }
    ...
    private function renderBlock(array $block, array $registry, array $context, ?DatasourceResolver $datasource, array &$assets, array &$seen, array &$diagnostics, ?array $parentLayout, string $path): string
    {
        ...
        if (! is_array($definition)) { $diagnostics[] = (new Diagnostic('unknown_block', 'warning', $path.'.type', $type))->toArray(); return ''; }
        $this->collectAssets($definition, $assets, $seen);
        $attrs = is_array($block['attrs'] ?? null) ? $block['attrs'] : [];
        foreach (($definition['attributes'] ?? []) as $key => $schema) if (is_array($schema) && ! array_key_exists($key, $attrs) && array_key_exists('default', $schema)) $attrs[$key] = $schema['default'];
        foreach (($block['bindings'] ?? []) as $attribute => $binding) {
            if (! is_string($attribute) || ! is_array($binding)) continue;
            $source = $binding['source'] ?? null; $value = null;
            try {
                if ($source === 'context') $value = ArrayPath::get($context, (string) ($binding['path'] ?? ''));
            ...
            } catch (Throwable $e) { $diagnostics[] = (new Diagnostic('datasource_error', 'warning', $path.'.bindings.'.$attribute, $e->getMessage()))->toArray(); }
        ...
        $children = '';
        foreach (($block['children'] ?? []) as $index => $child) if (is_array($child)) $children .= $this->renderBlock($child, $registry, $context, $datasource, $assets, $seen, $diagnostics, $layout, $path.'.children['.$index.']');
        $template = isset($definition['_template']) ? (string) file_get_contents($definition['_template']) : (string) ($definition['template'] ?? '');
        $rendered = $this->templates->render($template, ['attrs' => $attrs, 'context' => $context, 'children' => $children, 'blockId' => $block['id'] ?? '', 'slot' => $block['slot'] ?? null, 'preview' => false]);
    ...
    {
        $id = (string) ($block['id'] ?? ''); $safeId = $this->escape($id);
        ...
        $inlineStyle = trim($style['style'].';'.$layout['style'], ';'); $css = $style['css'].$layout['css'];
    ...
    private function wrapPage(array $page, string $body): string
    {
        ...
        $schemes = [];
        foreach (($settings['colorSchemes'] ?? []) as $scheme) if (is_array($scheme) && is_string($scheme['id'] ?? null) && is_array($scheme['colors'] ?? null)) $schemes[$scheme['id']] = $scheme;
        ...
        foreach (($settings['tokens'] ?? []) as $name => $value) if (is_string($name) && is_scalar($value)) $style[] = '--pb-'.$this->identifier($name).':'.$this->cssValue((string) $value);
        $out = '<div class="'.$this->escape($class).'" style="'.$this->escape(implode(';', $style)).'">'.$body.'</div>';
        $schemeCss = '';
        foreach ($schemes as $id => $scheme) { $decl = ''; foreach (($scheme['colors'] ?? []) as $name => $value) if (is_string($name) && is_scalar($value)) $decl .= '--pb-color-'.$this->identifier($name).':'.$this->cssValue((string) $value).';'; if ($decl !== '') $schemeCss .= '.pb-color-scheme--'.$this->identifier((string) $id).'{'.$decl.'background-color:var(--pb-color-background);color:var(--pb-color-foreground);}'; }
        $typography = $this->typographyCss($settings['typography'] ?? null); $custom = is_string($settings['customCss'] ?? null) ? $this->safeStyleBody($settings['customCss']) : '';
        if ($schemeCss !== '') $out .= '<style data-pb-color-schemes>'.$schemeCss.'</style>'; if ($typography !== '') $out .= '<style data-pb-typography>'.$typography.'</style>'; if ($custom !== '') $out .= '<style data-pb-page-css>'.$custom.'</style>';
        return $out;
    }
    ...
    private function typographyCss(mixed $typography): string
    {
        ...
        $families = is_array($typography['families'] ?? null) ? $typography['families'] : []; $styles = is_array($typography['styles'] ?? null) ? $typography['styles'] : [];
        ...
        $root = ''; foreach ($defaults as $name => $fallback) { $value = is_string($families[$name] ?? null) && trim($families[$name]) !== '' ? $families[$name] : $fallback; $root .= '--pb-font-'.$name.':'.$this->cssValue($value).';'; }
        ...
        foreach ($selectors as $name => $selector) { $s = is_array($styles[$name] ?? null) ? $styles[$name] : []; if ($s === []) continue; $family = in_array($s['family'] ?? null, ['primary','secondary','monospace'], true) ? $s['family'] : 'primary'; $decl = 'font-family:var(--pb-font-'.$family.');'; foreach (['size'=>'font-size','weight'=>'font-weight','lineHeight'=>'line-height','letterSpacing'=>'letter-spacing','textTransform'=>'text-transform'] as $key=>$property) if (is_string($s[$key] ?? null) && $s[$key] !== '') $decl .= $property.':'.$this->cssValue($s[$key]).';'; $css .= '.pb-page :is('.$selector.'){'.$decl.'}'; }
        return $css;
    }

    private function collectAssets(array $definition, array &$assets, array &$seen): void { foreach (['css','js'] as $group) foreach (($definition['assets'][$group] ?? []) as $asset) if (is_string($asset) && ! isset($seen[$group][$asset])) { $seen[$group][$asset] = true; $assets[$group][] = $asset; } }
