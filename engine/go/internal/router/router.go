...
func New(d Dependencies) http.Handler {
	...
	mux.HandleFunc("GET /health", func(w http.ResponseWriter, r *http.Request) { response.JSON(w, 200, map[string]string{"status": "ok"}) })
	mux.HandleFunc("GET /ready", func(w http.ResponseWriter, r *http.Request) {
		ctx, cancel := context.WithTimeout(r.Context(), 2*time.Second)
		defer cancel()
		sqlDB, err := d.DB.DB()
		if err != nil || sqlDB.PingContext(ctx) != nil {
			response.Error(w, 503, "not_ready", "database unavailable")
			return
		}
		response.JSON(w, 200, map[string]string{"status": "ready"})
	})
	...
	return middleware.Chain(mux, middleware.Recover, middleware.Logger, middleware.Security, middleware.CORS(d.CORSOrigins))
}
