...
func (s *Service) Get(ctx context.Context, t string) (map[string]any, error) {
	r, err := s.Registry(ctx)
	if err != nil {
		return nil, err
	}
	v, ok := r[t]
	if !ok {
		return nil, fmt.Errorf("block %q not found", t)
	}
	return v, nil
}
