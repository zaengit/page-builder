...
func Load() (Config, error) {
	...
	if cfg.HTTPAddress == "" { return Config{}, fmt.Errorf("HTTP_ADDR cannot be empty") }
	if cfg.DatabaseDSN == "" { return Config{}, fmt.Errorf("DB_DSN cannot be empty") }
	switch cfg.DatabaseDriver { case "sqlite", "postgres", "mysql", "sqlserver": default: return Config{}, fmt.Errorf("unsupported DB_DRIVER %q", cfg.DatabaseDriver) }
	if cfg.MaxUploadBytes <= 0 { return Config{}, fmt.Errorf("MAX_UPLOAD_BYTES must be positive") }
	if !strings.HasPrefix(cfg.PublicStoragePath,"/") { return Config{}, fmt.Errorf("PUBLIC_STORAGE_PATH must begin with /") }
	return cfg,nil
}
func env(k,d string) string { if v:=strings.TrimSpace(os.Getenv(k));v!=""{return v};return d }
func envBool(k string,d bool) bool { v:=strings.TrimSpace(os.Getenv(k));if v==""{return d};b,err:=strconv.ParseBool(v);if err!=nil{return d};return b }
...
func envDurationMS(k string,d int64) time.Duration { return time.Duration(envInt64(k,d))*time.Millisecond }
func splitCSV(v string)[]string{if strings.TrimSpace(v)==""{return nil};parts:=strings.Split(v,",");out:=make([]string,0,len(parts));for _,p:=range parts{if p=strings.TrimSpace(p);p!=""{out=append(out,p)}};return out}
