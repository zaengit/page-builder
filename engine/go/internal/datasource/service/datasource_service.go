...
func (s *Service) Query(ctx context.Context, in Query) (Result, error) {
	...
	if d == nil {
		return Result{}, errors.New("datasource resource not found")
	}
	cfg, err := parseConfig(d)
	if err != nil {
		return Result{}, err
	...
	for _, f := range in.Filters {
		...
		default:
			return Result{}, fmt.Errorf("operator %q is not allowed", f.Op)
		}
	}
	var total int64
	...
	if in.OrderBy != "" {
		if !allowed[in.OrderBy] {
			return Result{}, fmt.Errorf("order field %q is not allowed", in.OrderBy)
		}
		dir := strings.ToUpper(in.Direction)
		if dir != "DESC" {
			dir = "ASC"
		}
		q = q.Order(in.OrderBy + " " + dir)
	}
