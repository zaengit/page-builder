...
func (h *Handler) Get(w http.ResponseWriter, r *http.Request) {
	...
	v, err := h.service.Get(r.Context(), uint(id))
	if err != nil {
		response.Error(w, 404, "not_found", "media not found")
		return
	}
	response.JSON(w, 200, v)
}
func (h *Handler) Upload(w http.ResponseWriter, r *http.Request) {
	...
	if err := r.ParseMultipartForm(h.max); err != nil {
		response.Error(w, 413, "upload_too_large", err.Error())
		return
	}
	f, hdr, err := r.FormFile("file")
	if err != nil {
		response.Error(w, 400, "invalid_upload", err.Error())
		return
	}
	defer f.Close()
	m, err := h.service.Save(r.Context(), hdr.Filename, hdr.Header.Get("Content-Type"), f, hdr.Size)
	if err != nil {
		response.Error(w, 422, "invalid_upload", err.Error())
		return
	}
...
func (h *Handler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.service.Delete(r.Context(), uint(id)); err != nil {
		response.Error(w, 500, "delete_failed", err.Error())
		return
	}
	w.WriteHeader(204)
}
