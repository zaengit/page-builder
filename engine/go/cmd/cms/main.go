...
func main() {
	if len(os.Args)>1&&os.Args[1]=="--version"{fmt.Printf("page-builder-cms %s specification=%d\n",pagebuilder.EngineVersion,pagebuilder.SpecificationVersion);return}
	cfg,err:=config.Load();if err!=nil{slog.Error("configuration error","error",err);os.Exit(2)}
	db,err:=database.Open(cfg);if err!=nil{slog.Error("database error","error",err);os.Exit(3)};defer func(){if err:=database.Close(db);err!=nil{slog.Error("database close error","error",err)}}()
	if cfg.MigrateOnStart{if err:=database.Migrate(db);err!=nil{slog.Error("migration error","error",err);os.Exit(4)}}
	pr:=pagerepo.New(db);ps:=pagesvc.New(pr)
	bs:=blocksvc.New(cfg.BlockRoot)
	mr:=mediarepo.New(db);ms:=mediasvc.New(mr,cfg.StoragePath,cfg.PublicStoragePath,cfg.MaxUploadBytes)
	sr:=settingrepo.New(db);ss:=settingsvc.New(sr)
	dr:=datasourcerepo.New(db);ds:=datasourcesvc.New(dr)
	...
	errCh:=make(chan error,1);go func(){slog.Info("Go CMS started","addr",cfg.HTTPAddress,"database",cfg.DatabaseDriver);errCh<-srv.ListenAndServe()}()
	stop:=make(chan os.Signal,1);signal.Notify(stop,syscall.SIGINT,syscall.SIGTERM)
	select{case sig:=<-stop:slog.Info("shutdown requested","signal",sig.String());case err:=<-errCh:if err!=nil&&err!=http.ErrServerClosed{slog.Error("server error","error",err);os.Exit(5)};return}
	ctx,cancel:=context.WithTimeout(context.Background(),cfg.ShutdownTimeout);defer cancel();if err:=srv.Shutdown(ctx);err!=nil{slog.Error("shutdown error","error",err);os.Exit(6)};slog.Info("Go CMS stopped")
}
