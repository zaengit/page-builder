...
final class BlockRegistryLoader
{
    ...
    public function load(array $roots): array
    {
        ...
        foreach ($roots as $configured) {
            ...
            if ($root === false || isset($seenRoots[$root])) {
                continue;
            }
            $seenRoots[$root] = true;
            ...
            foreach (glob($root.'/*/block.json') ?: [] as $manifestPath) {
                ...
                if ($real === false || ! str_starts_with($real, $root.DIRECTORY_SEPARATOR)) {
                    throw new RuntimeException('unsafe_manifest_path');
                }
                ...
                if (! is_string($name) || preg_match('/^[a-z0-9][a-z0-9-]*(\/[a-z0-9][a-z0-9-]*)*$/', $name) !== 1) {
                    throw new RuntimeException('invalid_block_name');
                }
                if (isset($registry[$name])) {
                    throw new RuntimeException('duplicate_block:'.$name);
                }

                $directory = dirname($real);
                $template = realpath($directory.'/template.html');
                if ($template === false || ! str_starts_with($template, $directory.DIRECTORY_SEPARATOR)) {
                    throw new RuntimeException('missing_portable_template:'.$name);
                }
                ...
                foreach (['css', 'js'] as $extension) {
                    foreach (($manifest['assets'][$extension] ?? []) as $asset) {
                        if (! is_string($asset) || preg_match('/^[A-Za-z0-9._-]+$/', $asset) !== 1 || strtolower(pathinfo($asset, PATHINFO_EXTENSION)) !== $extension || realpath($directory.'/'.$asset) === false) {
                            throw new RuntimeException('invalid_asset:'.$name);
                        }
                    }
                }
