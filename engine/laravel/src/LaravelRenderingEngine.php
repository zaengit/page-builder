...
final class LaravelRenderingEngine implements RenderingEngine
{
    ...
    {
        $result = $this->runtime->render($page, $this->blockPaths(), $this->runtimeContext(), null, $this->datasources);
    ...
    private function runtimeContext(): array
    {
        ...
        $request = request(); $explicit = $request->attributes->get('page_builder_context', []); $route = $request->route(); $routeParameters = is_object($route) && method_exists($route, 'parameters') ? $route->parameters() : []; $preview = $request->input('context', []);
        return array_replace_recursive(is_array($routeParameters) ? $routeParameters : [], is_array($explicit) ? $explicit : [], is_array($preview) ? $preview : []);
    }
}
