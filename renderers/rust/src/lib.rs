...
impl UniversalRenderer {
    ...
    fn render_block(
        ...
        let Some(definition) = request.registry.get(block_type) else {
            result.diagnostics.push(format!("unknown_block:{block_type}"));
            return String::new();
        };
        ...
        if let Some(attributes) = definition.get("attributes").and_then(Value::as_object) {
            for (key, schema) in attributes {
                if let Some(default) = schema.as_object().and_then(|value| value.get("default")) {
