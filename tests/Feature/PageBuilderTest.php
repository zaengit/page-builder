...
class PageBuilderTest extends TestCase
{
    ...
    {
        $this->getJson('/api/page-builder/blocks')
            ->assertOk()
            ->assertJsonFragment(['name'=>'core/heading']);

        $this->postJson('/api/page-builder/render-page', ['blocks'=>[]])
            ->assertOk()
            ->assertJsonStructure(['html','assets']);
    }
    ...
    {
        $this->get('/page-builder/preview')
            ->assertOk()
            ->assertSee('Page Builder Preview')
            ->assertSee('pb-canvas');
    }
}
