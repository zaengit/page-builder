...
final class LayoutSerializer
{
    ...
    {
        $base = array_merge(
            $this->containerRules($layout, 'desktop'),
            $this->itemRules($layoutItem, $parentLayout, 'desktop'),
        );
        $tablet = array_merge(
            $this->containerRules($layout, 'tablet'),
            $this->itemRules($layoutItem, $parentLayout, 'tablet'),
        );
        $mobile = array_merge(
            $this->containerRules($layout, 'mobile'),
            $this->itemRules($layoutItem, $parentLayout, 'mobile'),
        );
        ...
        if ($tablet !== []) {
            $css .= '@media(max-width:1024px){'.$selector.'{'.implode(';', $tablet).'}}';
        }

        if ($mobile !== []) {
            $css .= '@media(max-width:640px){'.$selector.'{'.implode(';', $mobile).'}}';
        }
    ...
    private function itemRules(array $item, ?array $parentLayout, string $breakpoint): array
    {
        ...
        if ($parentMode === 'grid') {
            ...
            return [
                'grid-column:'.($columnStart === 'auto'
                    ? 'span '.$columnSpan
                    : max(1, (int) $columnStart).' / span '.$columnSpan),
                'grid-row:'.($rowStart === 'auto'
                    ? 'span '.$rowSpan
                    : max(1, (int) $rowStart).' / span '.$rowSpan),
            ];
        }
