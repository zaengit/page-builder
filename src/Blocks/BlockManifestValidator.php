...
final class BlockManifestValidator
{
    public function validate(array $manifest, string $manifestPath): void
    {
        ...
        foreach ($attributes as $attribute => $schema) {
            $this->validateAttributeSchema($schema, (string) $attribute, $name);
        }
    ...
    private function validateAttributeSchema(mixed $schema, string $attribute, string $name): void
    {
        ...
        foreach ($fields as $field => $fieldSchema) {
            $this->validateAttributeSchema($fieldSchema, $attribute.'.'.$field, $name);
        }
    }
    ...
    private function validateSupports(mixed $supports, string $name): void
    {
        ...
        if (array_key_exists('children', $supports) && ! is_bool($supports['children'])) {
            throw new RuntimeException("Invalid children support for {$name}");
        }
