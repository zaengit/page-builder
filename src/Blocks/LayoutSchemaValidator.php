...
final class LayoutSchemaValidator
{
    ...
    {
        if (! is_array($layout)) {
            self::fail($path, 'Layout must be an object.');
        }
        ...
        foreach ($layout as $key => $value) {
            $result[$key] = self::responsive(
                $value,
                $path.'.'.$key,
                fn (mixed $item, string $itemPath) => self::sectionValue($key, $item, $itemPath),
            );
        }
    ...
    {
        if (! is_array($item)) {
            self::fail($path, 'Layout item must be an object.');
        }
        ...
        foreach ($item as $key => $value) {
            $result[$key] = self::responsive(
                $value,
                $path.'.'.$key,
                fn (mixed $entry, string $itemPath) => self::itemValue($key, $entry, $itemPath),
            );
        }
    ...
    {
        if (! is_array($value)) {
            self::fail($path, 'Responsive layout values must be objects.');
        }
        ...
        foreach ($value as $breakpoint => $entry) {
            if (! in_array($breakpoint, self::BREAKPOINTS, true)) {
                self::fail($path, 'Unknown responsive breakpoint.');
            }
    ...
    private static function knownKeys(array $value, array $allowed, string $path): void
    {
        foreach (array_keys($value) as $key) {
            if (! is_string($key) || ! in_array($key, $allowed, true)) {
                self::fail($path.'.'.$key, 'Unknown layout property.');
            }
        }
    }
    ...
    private static function enum(mixed $value, array $allowed, string $path): string
    {
        if (! is_string($value) || ! in_array($value, $allowed, true)) {
            self::fail($path, 'Invalid layout option.');
        }
    ...
    private static function integer(mixed $value, int $min, int $max, string $path): int
    {
        if (! is_int($value) || $value < $min || $value > $max) {
            self::fail($path, "Value must be an integer between {$min} and {$max}.");
        }
    ...
    private static function number(mixed $value, float $min, float $max, string $path): int|float
    {
        if ((! is_int($value) && ! is_float($value)) || $value < $min || $value > $max) {
            self::fail($path, "Value must be between {$min} and {$max}.");
        }
    ...
    private static function shortString(mixed $value, string $path): string
    {
        if (! is_string($value) || mb_strlen($value) > 100) {
            self::fail($path, 'Layout CSS value must be a string up to 100 characters.');
        }
