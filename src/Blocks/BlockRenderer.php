...
final class BlockRenderer
{
    ...
    public function render(array $block, bool $preview = false, string $children = ''): string
    {
        ...
        $attrs = $block['attrs'] ?? [];
        foreach (($definition['attributes'] ?? []) as $key => $schema) if (! array_key_exists($key, $attrs) && array_key_exists('default', $schema)) $attrs[$key] = $schema['default'];

        $context = new BlockRenderContext((string) ($block['id'] ?? ''), (string) ($block['type'] ?? ''), $attrs, null, $preview);
        $attrs = $this->bindings->resolve($attrs, is_array($block['bindings'] ?? null) ? $block['bindings'] : [], $context);
