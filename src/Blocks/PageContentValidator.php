...
final class PageContentValidator
{
    ...
    public function validate(array $content): array
    {
        ...
        if (! is_array($blocks) || ! array_is_list($blocks)) {
            $this->fail('blocks', 'The blocks field must be a list.');
        }
        ...
        foreach ($blocks as $index => $block) {
            $validated[] = $this->validateBlock($block, "blocks.{$index}", 0, $seenIds, $count);
        }

        $result = [
            'blocks' => $validated,
            'schemaVersion' => max(1, (int) ($content['schemaVersion'] ?? 1)),
        ];

        if (array_key_exists('settings', $content)) {
            $result['settings'] = $this->validateSettings($content['settings']);
        }
    ...
    private function validateBlock(mixed $block, string $path, int $depth, array &$seenIds, int &$count): array
    {
        ...
        if (! is_string($id) || $id === '' || mb_strlen($id) > 100) {
            $this->fail("{$path}.id", 'Block id must be a non-empty string up to 100 characters.');
        }

        if (isset($seenIds[$id])) {
            $this->fail("{$path}.id", 'Block ids must be unique within a page.');
        }

        $seenIds[$id] = true;
        $type = $block['type'] ?? null;

        if (! is_string($type) || $type === '' || mb_strlen($type) > 100) {
            $this->fail("{$path}.type", 'Block type must be a non-empty string up to 100 characters.');
        }

        $definition = $this->registry->get($type);

        if (! $definition) {
            $this->fail("{$path}.type", "Unknown block type [{$type}].");
        }
        ...
        if (! is_array($attrs)) {
            $this->fail("{$path}.attrs", 'Block attrs must be an object.');
        }

        foreach ($attrs as $name => $value) {
            if (! is_string($name) || ! isset($definition['attributes'][$name])) {
                $this->fail("{$path}.attrs.{$name}", 'Unknown block attribute.');
            }
        ...
        if (! is_array($children) || ! array_is_list($children)) {
            $this->fail("{$path}.children", 'Block children must be a list.');
        }

        if ($children !== [] && ! ($definition['supports']['children'] ?? false)) {
            $this->fail("{$path}.children", 'This block type does not support children.');
        }
        ...
        $slotDefinitions = is_array($definition['supports']['slots'] ?? null)
            ? $definition['supports']['slots']
            : [];
        $slots = [];

        foreach ($slotDefinitions as $slotDefinition) {
            if (is_array($slotDefinition) && is_string($slotDefinition['name'] ?? null)) {
                $slots[$slotDefinition['name']] = $slotDefinition;
            }
        }
        ...
        foreach ($children as $index => $child) {
            if (is_array($child) && isset($child['slot'])) {
                ...
                if (! $slot) {
                    $this->fail("{$path}.children.{$index}.slot", 'Unknown parent slot.');
                }

                if (($slot['allowedChildren'] ?? []) !== [] && ! in_array($child['type'] ?? null, $slot['allowedChildren'], true)) {
                    $this->fail("{$path}.children.{$index}.type", 'This block type is not allowed in the selected slot.');
                }
            }

            $validatedChildren[] = $this->validateBlock(
                $child,
                "{$path}.children.{$index}",
                $depth + 1,
                $seenIds,
        ...
        if (array_key_exists('children', $block) || ($definition['supports']['children'] ?? false)) {
        ...
        if (isset($block['styles'])) {
            $validated['styles'] = $this->validateStyles($block['styles'], "{$path}.styles");
        }

        if (isset($block['bindings'])) {
            $validated['bindings'] = $this->validateBindings($block['bindings'], $definition, "{$path}.bindings");
        }

        if (isset($block['lock'])) {
            $validated['lock'] = $this->validateLock($block['lock'], "{$path}.lock");
        }
    ...
    private function validateAttribute(mixed $value, array $schema, string $path): void
    {
        if (($schema['responsive'] ?? false) === true) {
            if (! is_array($value)) {
                $this->fail($path, 'Responsive attributes must be an object.');
            }

            foreach ($value as $breakpoint => $item) {
                if (! in_array($breakpoint, ['desktop', 'tablet', 'mobile'], true)) {
                    $this->fail($path, 'Unknown responsive breakpoint.');
                }

                $next = $schema;
                $next['responsive'] = false;
                $this->validateAttribute($item, $next, $path.'.'.$breakpoint);
            }
        ...
        if (($type === 'number' || $type === 'range') && is_numeric($value)) {
            ...
            if (isset($schema['max']) && $value > $schema['max']) {
                $this->fail($path, "Value may not be greater than {$schema['max']}.");
            }
        }
    }
    ...
    private function validRepeater(mixed $value, array $schema, string $path): bool
    {
        ...
        foreach ($value as $index => $item) {
            if (! is_array($item)) {
                $this->fail("{$path}.{$index}", 'Repeater items must be objects.');
            }

            foreach ($item as $name => $fieldValue) {
                if (! is_string($name) || ! isset($fields[$name])) {
                    $this->fail("{$path}.{$index}.{$name}", 'Unknown repeater field.');
                }
    ...
    private function validateBindings(mixed $bindings, array $definition, string $path): array
    {
        if (! is_array($bindings)) {
            $this->fail($path, 'Bindings must be an object.');
        }

        foreach ($bindings as $attr => $binding) {
            if (! isset($definition['attributes'][$attr]) || ! is_array($binding) || ! is_string($binding['source'] ?? null)) {
    ...
    {
        if (! is_array($lock)) {
            $this->fail($path, 'Lock must be an object.');
        }
        ...
        foreach (['move', 'remove', 'edit'] as $key) {
            ...
            if (! is_bool($lock[$key])) {
                $this->fail($path.'.'.$key, 'Lock values must be boolean.');
            }
    ...
    private function validateSettings(mixed $settings): array
    {
        if (! is_array($settings)) {
            $this->fail('settings', 'Page settings must be an object.');
        }
        ...
        foreach (array_keys($settings) as $key) {
            if (! in_array($key, $allowed, true)) {
                $this->fail('settings.'.$key, 'Unknown page setting.');
            }
        }
