...
export function useEditorMessages(mediaRequest: MutableRefObject<MediaRequest>, actions: MessageActions) {
  useEffect(() => {
    const listener = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'PAGE_BUILDER_MEDIA_SELECTED' && mediaRequest.current && event.data.url) {
        actions.updateAttrPath(mediaRequest.current.id, mediaRequest.current.path, event.data.url);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        setError('');
        iframe.current?.contentWindow?.postMessage({ type: 'REPLACE_PAGE', html: result.html, assets: result.assets }, location.origin);
      } catch (error) {
...
export async function readClipboardBlock(fallback: PageBlock | null): Promise<PageBlock | null> {
  ...
  try {
    ...
    if (text) {
      ...
      if (parsed?.pageBuilderBlock) block = parsed.pageBuilderBlock as PageBlock;
    }
  } catch {
    // Clipboard permission is optional; internal clipboard remains available.
  }
  return block;
}
