...
function Help({ text }: { text?: string }) { return text ? <p className="text-[11px] leading-snug text-muted-foreground/85">{text}</p> : null; }
function Field({ label, htmlFor, help, children }: { label: string; htmlFor?: string; help?: string; children: React.ReactNode }) { return <div className="grid gap-1.5"><Label htmlFor={htmlFor} className="text-xs font-medium">{label}</Label>{children}<Help text={help} /></div>; }
...
function BasicControl({ name, path = [name], schema, value, onChange, requestMedia }: ControlProps) {
  ...
  if (schema.type === 'repeater') {
    ...
    return <div className="grid gap-3"><div className="flex items-center justify-between gap-2"><div className="grid gap-1"><Label className="text-xs">{label}</Label><Help text={schema.help} /></div><Button type="button" size="xs" variant="secondary" onClick={() => onChange([...items, defaults(schema.fields)])}><Plus />Add</Button></div>{items.length === 0 && <div className="rounded-lg bg-muted/35 p-3 text-center text-xs text-muted-foreground">No items yet</div>}{items.map((item, index) => <Card key={index} className="gap-2 rounded-xl border-border/40 bg-muted/20 py-3 shadow-none"><CardHeader className="px-3"><div className="flex items-center justify-between gap-2"><CardTitle className="text-xs">Item {index + 1}</CardTitle><Button type="button" size="icon-xs" variant="ghost" className="editor-icon-button" onClick={() => onChange(items.filter((_, itemIndex) => itemIndex !== index))} aria-label={`Remove item ${index + 1}`}><Trash2 /></Button></div></CardHeader><Separator className="editor-divider" /><CardContent className="grid gap-3 px-3">{Object.entries(schema.fields ?? {}).map(([field, fieldSchema]) => <Control key={field} name={field} path={[...path, String(index), field]} schema={fieldSchema} value={item[field]} attrs={item} onChange={next => onChange(items.map((current, itemIndex) => itemIndex === index ? { ...current, [field]: next } : current))} requestMedia={requestMedia} />)}</CardContent></Card>)}</div>;
  }
  if (schema.type === 'image') { const url = String(value ?? ''); return <Field label={label} htmlFor={id} help={schema.help}><div className="grid gap-2">{url && <Card className="overflow-hidden rounded-xl border-border/40 p-0 shadow-none"><img src={url} alt="" className="h-24 w-full object-cover" /></Card>}<div className="flex gap-2"><Input id={id} className="h-9 text-sm" type="url" value={url} onChange={event => onChange(event.target.value)} placeholder="https://…" />{requestMedia && <Button type="button" variant="secondary" size="icon-sm" onClick={() => requestMedia(path)} aria-label={`Browse ${label}`}><ImagePlus /></Button>}</div></div></Field>; }
...
export function Control(props: ControlProps) {
  ...
  return <div className="grid gap-2 pt-1">
    <div className="flex items-center gap-2">
      <p className="editor-section-label">{props.schema.group}</p>
      <Separator className="editor-divider flex-1" />
    </div>
    {control}
  </div>;
}
