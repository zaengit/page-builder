...
export function SectionLayoutInspector({ block, breakpoint, onChange }: { block: PageBlock; breakpoint: Breakpoint; onChange: (layout: SectionLayout) => void }) {
  ...
  return (
    <div className="space-y-3">
      <div className="grid gap-1"><Label className="text-[11px]">Layout</Label><ToggleGroup type="single" value={mode} onValueChange={v => v && set('mode', v)} className="justify-start"><ToggleGroupItem value="block">Block</ToggleGroupItem><ToggleGroupItem value="flex">Flex</ToggleGroupItem><ToggleGroupItem value="grid">Grid</ToggleGroupItem></ToggleGroup></div>
      {mode === 'flex' && <>
...
function FieldSelect({ label, value, onChange, options }: { label: string; value: string; onChange: (value: string) => void; options: string[] }) { return <div className="grid gap-1"><Label className="text-[11px]">{label}</Label><Select value={value} onValueChange={onChange}><SelectTrigger size="sm"><SelectValue /></SelectTrigger><SelectContent>{options.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select></div> }
function TextField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { return <div className="grid gap-1"><Label className="text-[11px]">{label}</Label><Input className="h-7 text-xs" value={value} onChange={e => onChange(e.target.value)} /></div> }
function NumberField({ label, value, min, max, onChange }: { label: string; value: number; min: number; max: number; onChange: (value: number) => void }) { return <div className="grid gap-1"><Label className="text-[11px]">{label}</Label><Input className="h-7 text-xs" type="number" min={min} max={max} value={value} onChange={e => onChange(Math.min(max, Math.max(min, Number(e.target.value) || min)))} /></div> }
