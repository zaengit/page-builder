...
function ensureAssets(assets = {}) {
  for (const href of assets.css ?? []) {
    ...
    const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = href; link.dataset.pbAsset = 'css'; document.head.appendChild(link);
  }
  for (const src of assets.js ?? []) {
    ...
    const script = document.createElement('script'); script.type = 'module'; script.src = src; script.dataset.pbAsset = 'js'; document.body.appendChild(script);
  }
}
document.addEventListener('click', event => {
  const block = event.target.closest('[data-block-id]');
  if (block) window.parent.postMessage({type:'SELECT_BLOCK', blockId:block.dataset.blockId}, allowedOrigin);
});
